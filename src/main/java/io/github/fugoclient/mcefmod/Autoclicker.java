...
public class Autoclicker {
    ...
    public static void onKey(int key, int action) {
        ...
        if (action == GLFW.GLFW_PRESS && key == keybindCode) {
            ...
            if (client.currentScreen == null) {
                ...
                // Send actionbar message to notify player
                if (client.player != null) {
                    client.player.sendMessage(
                        Text.of("§8[§6Aether§8] §fAutoclicker: " + (active ? "§aON" : "§cOFF")),
    ...
    public static void tick(MinecraftClient client) {
        ...
        // Check if the corresponding mouse button/key is held down
        boolean isHeld = false;
        if ("left".equals(button)) {
            isHeld = client.options.attackKey.isPressed();
        } else if ("right".equals(button)) {
            isHeld = client.options.useKey.isPressed();
        }
